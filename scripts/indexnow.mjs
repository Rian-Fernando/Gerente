...
const HOST = 'gerente.rianfernando.com';
const KEY = '08378907a9bc700d4fb149f367324e46';
const KEY_LOCATION = `https://${HOST}/${KEY}.txt`;
const SITEMAP = `https://${HOST}/sitemap.xml`;
const ENDPOINT = 'https://api.indexnow.org/indexnow';

async function main() {
  const res = await fetch(SITEMAP, { headers: { 'User-Agent': 'gerente-indexnow/1.0' } });
  ...
  console.log(
    `Submitting ${urlList.length} URL(s) from ${SITEMAP}` +
